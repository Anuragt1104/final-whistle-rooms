...
Map<String, dynamic> player(int i, {bool starter = true}) => {
      ...
      'stats': {'goals': i == 9 ? 1 : 0, 'starts': starter ? 1 : 0, 'squadSelections': 1},
    };
...
void main() {
  test('match intelligence parses confirmed XI and attributed event', () {
    final data = MatchData.fromJson({
      ...
      'stale': false,
      'fixture': {
        'id': '18237038', 'status': 'live',
        'home': {'id': 'fra', 'name': 'France', 'code': 'FRA', 'flag': '🇫🇷'},
        'away': {'id': 'esp', 'name': 'Spain', 'code': 'ESP', 'flag': '🇪🇸'},
      },
      'teams': {
        'home': {'id': 'fra', 'name': 'France', 'code': 'FRA', 'formation': '4-3-3', 'players': List.generate(11, (i) => player(i + 1))},
        'away': {'id': 'esp', 'name': 'Spain', 'code': 'ESP', 'formation': '4-3-3', 'players': List.generate(11, (i) => player(i + 21))},
      },
      'events': [
        {'id': 'tx:1', 'sourceEventId': 'tx:1', 'kind': 'goal', 'side': 'home', 'teamCode': 'FRA', 'playerId': 'p9', 'playerName': 'Verified Player 9', 'label': 'Goal', 'minute': 31, 'seq': 9, 'ts': 1},
      ],
    });
  ...
  test('Official Hub metadata is backward-compatible', () {
    final summary = RoomSummary.fromJson({
      'id': 'hub1', 'code': 'ABC123', 'name': 'Official', 'status': 'live', 'kind': 'official', 'autoManaged': true, 'memberCount': 2,
      'fixture': {
        'id': 'fx', 'home': {'id': 'fra', 'name': 'France', 'code': 'FRA'}, 'away': {'id': 'esp', 'name': 'Spain', 'code': 'ESP'},
      },
    });
  ...
  test('shared motion controller supports deterministic drag fallback', () {
    ...
    expect(motion.x, isNot(0));
    expect(motion.y, isNot(0));
    motion.resetDrag();
    motion.dispose();
  });
}
