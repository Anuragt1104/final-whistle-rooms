...
/// on any failure.
class SolanaRpc {
  static String _endpoint(String cluster) =>
      cluster == 'mainnet-beta' ? 'https://api.mainnet-beta.solana.com' : 'https://api.devnet.solana.com';

  static Future<double?> balance(String pubkey, {String cluster = 'mainnet-beta'}) async {
    if (pubkey.isEmpty) return null;
    try {
      final res = await http
          .post(Uri.parse(_endpoint(cluster)),
              headers: {'content-type': 'application/json'},
              body: jsonEncode({'jsonrpc': '2.0', 'id': 1, 'method': 'getBalance', 'params': [pubkey]}))
