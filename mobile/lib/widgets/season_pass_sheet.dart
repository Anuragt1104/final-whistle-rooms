        ...
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          ...
          Container(
            ...
            child: Column(children: [
              ...
              Text('SEASON PASS', style: display(28, color: AppColors.cream, spacing: 1)),
              const SizedBox(height: 4),
              Text('One pass. Every match of the tournament.',
                  textAlign: TextAlign.center, style: body(color: AppColors.mutInk, size: 12.5)),
            ]),
          ),
          const SizedBox(height: 18),
          _perk('💎', 'Pro reaction pack', '10 exclusive reactions — 🐐 👑 🥶 💯 and more — in every room you join'),
          _perk('🛡️', 'Supporter badge', '★ PRO on your leaderboard row + gold badge on your profile'),
          _perk('⚡', 'Priority rooms', 'Front of the queue when big matches fill up'),
          _perk('🔮', 'Coming next', 'Custom room themes and post-match super-recaps'),
          ...
          Container(
            ...
            child: Row(children: [
              Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text('TOURNAMENT PASS', style: label(color: AppColors.mut, size: 9)),
                Text('\$4.99', style: display(24, color: AppColors.ink)),
              ]),
              const Spacer(),
              Text('one-time · all 104 matches', style: body(color: AppColors.mut, size: 11.5)),
            ]),
          ),
          const SizedBox(height: 14),
          PrimaryButton(_busy ? 'Unlocking…' : 'Unlock Season Pass', icon: Icons.lock_open_rounded, expand: true, busy: _busy, onTap: _unlock),
          const SizedBox(height: 8),
          GhostButton('Not now', expand: true, onTap: () => Navigator.pop(context, false)),
          const SizedBox(height: 10),
          Center(child: Text('Skill-based, points only — the pass never buys an advantage.',
              textAlign: TextAlign.center, style: body(color: AppColors.mut, size: 10.5))),
        ]),
      ),
    );
  }
