  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        Row(children: [
          Text('TOURNAMENT', style: label(color: AppColors.ink, size: 11, weight: FontWeight.w800)),
          const SizedBox(width: 8),
          Container(
            padding: const EdgeInsets.symmetric(horizontal: 7, vertical: 2),
            decoration: BoxDecoration(color: AppColors.ink, borderRadius: BorderRadius.circular(99)),
            child: Text(_currentStage(played), style: label(color: AppColors.cream, size: 8, weight: FontWeight.w800)),
          ),
          const Spacer(),
          AnimatedCount(played, style: display(16, color: AppColors.orange)),
          Text(' of $total played', style: body(color: AppColors.mut, size: 11.5)),
          if (liveNow > 0) Text(' · $liveNow live', style: body(color: AppColors.orange, size: 11.5, weight: FontWeight.w800)),
        ]),
            ...
            SizedBox(
              ...
              child: Stack(children: [
                ...
                TweenAnimationBuilder<double>(
                  tween: Tween(begin: 0, end: (played / total).clamp(0.0, 1.0)),
                ...
                for (final (n, _) in _stageBoundaries.take(_stageBoundaries.length - 1))
                  Positioned(
                    left: w * n / total - 1,
                    top: 2,
                    bottom: 2,
                    child: Container(width: 2, color: played >= n ? const Color(0x66FFFFFF) : AppColors.line),
                  ),
              ]),
            ),
            ...
            SizedBox(
              ...
              child: Stack(children: [
                for (var i = 0; i < _stageBoundaries.length; i++)
                  Positioned(
                    left: i == 0 ? 0 : w * _stageBoundaries[i - 1].$1 / total,
                    width: i == 0 ? w * 72 / total : w * (_stageBoundaries[i].$1 - _stageBoundaries[i - 1].$1) / total,
                    child: Center(
                      child: Text(
                        _stageBoundaries[i].$2,
                        style: label(color: played >= _stageBoundaries[i].$1 ? AppColors.orange : AppColors.mut, size: 7, weight: FontWeight.w800),
                      ),
                    ),
                  ),
              ]),
